...
def get_first_name_of_season_winner(data, season)
  # code here
  season_contestants = data.fetch(season)
  season_winner = season_contestants.map do |contestants|
    if contestants.fetch("status") == "Winner"
      contestants.fetch("name")
    end
  end
  season_winner.compact.join().split.first
end

def get_contestant_name(data, occupation)
  # code here
  data.map do |season, info|
    contestants = info.map do |attribute, value|
      if attribute.fetch("occupation") == occupation
...
def count_contestants_by_hometown(data, hometown)
  ...
  data.each do |season, info|
    info.each do |attribute, value|
      if attribute.fetch("hometown") == hometown
        x += 1
      end
    end
  end
...
def get_occupation(data, hometown)
  # code here
  data.map do |season, info|
    info.map do |attribute, value|
      if attribute.fetch("hometown") == hometown
        attribute.fetch("occupation")
      end
    end.compact.first
  end.flatten.join()
end
...
def get_average_age_for_season(data, season)
  # code here
  season_contestants = data.fetch(season)
  string_array = season_contestants.map do |contestants|
    contestants.fetch("age")
  end
  average_age = string_array.map{|i| i.to_f}
  rounded = average_age.inject(0) {|sum,x| sum + x} / average_age.size
  rounded.round 
end
